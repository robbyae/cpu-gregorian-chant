...
document.getElementById("mode-dropdown").addEventListener("click", () => {
    ...
    {
        txt = 'Gregorian Chant is categorized in eight modes of which there are two varieties: authentic and plagal. Each mode has a relative and posesses three defining attributes: a finalis, a dominant, and an ambitus. Relative modes share the same finalis, but possess different dominants.';
...
{
    txt = 'Welcome to CPU Gregorian Chant, an application to procedurally generate medieval church song! Gregorian Chant refers to the primary repertory of plainsong, a type of monophonic song sung in liturgies of the Western Church. Hover over our illuminated words for more information, select a button on the right for an article, or choose a mode and form on the left to compose a Gregorian Chant.';
    if(txt != prev_txt){find_important();}
}
...
function initialize_performance(disable_perform)
{
    ...
    if(perform_button.innerHTML == "Sing")
    {
        is_performing = 1;
        // perform(voice);
