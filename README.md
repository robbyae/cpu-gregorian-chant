...
**Music Generation**
<br>
I believe it is important to note that the music is not generated by "AI". It is generated through an algorithm written in Python. To describe the process at a very high level — the music generates in the form of a string using [gabc](https://gregorio-project.github.io/gabc/) notation, a notation which uses ASCII characters to create Gregorian chant scores ("sheet music"). After the gabc has generated, the string is parsed, and a separate string is created to be used by [Tone=](https://tonejs.github.io/) for the production of sound. These strings are then passed from Python to JavaScript where they are processed by [exsurge](https://github.com/frmatthew/exsurge) and [Tone](https://tonejs.github.io/) to render notation and produce sound, respectively. Examples of potential strings are given below.
<br>
